...
bool loadMedia()
{
    ...
    bool success = true;
    gTexture = IMG_LoadTexture(gRenderer, "jonas.png");
    if( gTexture == NULL )
    {
        printf( "Failed to load texture image!\n" );
        success = false;
    }
    tTexture = IMG_LoadTexture(gRenderer, "store.png");
    if( tTexture == NULL )
    {
        printf( "Failed to load texture image!\n" );
        success = false;
...
int main(int argc, char *args[])
{
    ...
    else
    {
        ...
        else
        {
            ...
            pos storePos;
            storePos.x = SCREEN_WIDTH/4;
            storePos.y = SCREEN_HEIGHT/4;
            ...
            while (!quit)
            {
                ...
                SDL_Rect jonasQuad = { playerPos.x, playerPos.y, SCREEN_HEIGHT / 24, SCREEN_HEIGHT / 24};
                SDL_RenderCopy(gRenderer, gTexture, NULL, &jonasQuad);

                SDL_Rect storeQuad = { storePos.x, storePos.y, SCREEN_HEIGHT / 8, SCREEN_HEIGHT / 8};
                SDL_RenderCopy(gRenderer, tTexture, NULL, &storeQuad);
